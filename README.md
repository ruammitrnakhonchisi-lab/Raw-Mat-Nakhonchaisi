...
- **รับเข้า (Stock In)** — บันทึกรับวัตถุดิบพร้อม Lot/Batch, วันหมดอายุ, ราคา, เลขที่ PO, auto-fill ราคา/ผู้จำหน่ายจากทะเบียนวัตถุดิบ, เพิ่มวัตถุดิบใหม่ได้ทันทีจากหน้านี้ถ้ายังไม่มี SKU ในระบบ, ดูประวัติรับเข้าล่าสุดและยกเลิกรายการที่ผิดพลาดได้ (admin) — บันทึกแบบ atomic ทั้งหมดผ่าน Postgres function เดียว — **เฉพาะวัตถุดิบหมวด "PC wire"**: พิมพ์หรือวางเลข Coil จากใบส่งของ บรรทัดละ 1 เลข (วางได้ทีเดียวหลายม้วน ไม่ต้องกดเพิ่มแถวทีละอัน) นับหน่วยเป็น "ขด" (1 coil = 1 ขด ไม่ใช้น้ำหนัก)
- **เบิกออก (Stock Out)** — บันทึกเบิกใช้ พร้อมแผนก/เลขที่ใบสั่งผลิต ตรวจสอบยอดคงเหลือก่อนเบิกอัตโนมัติ — **เฉพาะวัตถุดิบหมวด "PC wire"**: ค้นหา/เลือกเบิกเจาะจงตาม Coil (อ่านเลขจากแท็กเหล็กที่ติดมากับม้วนลวด พิมพ์กรองในช่องค้นหาก่อนเลือกได้) เบิกทีละ 1 ขด ระบบตัดยอดคงเหลือของ coil นั้นโดยตรง ไม่ปนกับ coil อื่น พร้อมเลือก "ใช้งาน" ว่าเบิกไปผลิตอะไรจาก dropdown ที่กำหนดไว้ (กันพิมพ์ผิด)
...
- FEFO ระดับ lot สำหรับเบิกออก (ปัจจุบันตัดยอดรวมต่อ SKU ไม่ได้ตัดทีละ lot ยกเว้นวัตถุดิบหมวด "PC wire" ที่เบิกแบบเจาะจง coil ได้แล้ว)
